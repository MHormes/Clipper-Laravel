    ...
    <AppLayout :breadcrumbs="breadcrumbItems">
        ...
        <SettingsLayout>
            <div class="flex flex-col space-y-10 p-1">
                ...
                <Form
                    v-bind="NotificationsController.update.form()"
                    class="space-y-6"
                    v-slot="{ processing, recentlySuccessful }"
                >
                    <div v-if="isAdmin && adminCategories.length" class="rounded-2xl border border-border-color bg-component-background p-6 shadow-sm space-y-5">
                        <p class="text-xs font-semibold uppercase tracking-widest text-muted-content">Admin Notifications</p>
                        <div
                        ...
                        >
                            ...
                            <label class="relative inline-flex shrink-0 cursor-pointer items-center">
                                <input
                                    ...
                                    class="peer sr-only"
                                />
                                <div class="h-5 w-9 rounded-full bg-muted-background peer-checked:bg-[var(--primary)] transition-colors after:absolute after:left-[2px] after:top-[2px] after:h-4 after:w-4 after:rounded-full after:bg-white after:transition-all peer-checked:after:translate-x-4 after:shadow-sm"></div>
                            </label>
                        </div>
                    </div>
