...
import { ChevronLeft, CheckCircle2, Info } from '@lucide/vue';
...
<template>
    ...
    <AppLayout>
        <div class="max-w-6xl mx-auto p-4 md:p-6">
            <div class="mb-10">
                <Link :href="route('admin.requests.series.index')"
                ...
                <div class="flex items-center justify-between mt-4">
                    ...
                    <div class="flex items-center gap-3">
                        <button @click="declineSeries"
                            class="px-6 py-3 bg-error/10 text-error hover:bg-error hover:text-button-content!  rounded-xl font-bold uppercase tracking-widest text-xs transition-all border border-error/20">
                            Decline Request
                        </button>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1 space-y-6">
                    <div class="bg-component-background rounded-3xl border border-border-color p-6 shadow-sm">
                        <h2
                            class="text-xs font-black uppercase tracking-[0.2em] text-primary mb-6 flex items-center gap-2">
                            Series Overview
                        </h2>

                        <div
                            class="aspect-[4/3] rounded-2xl overflow-hidden mb-6 bg-media-bg border border-border-color">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label
                                    class="block text-[10px] font-black uppercase tracking-widest text-muted-content mb-1">Series
                                    Name</label>
                                <p class="text-lg font-black uppercase tracking-tight text-primary-content">{{
                                    series.name }}</p>
                            </div>
                            <div class="pt-4 border-t border-border-color">
                                <label
                                    class="block text-[10px] font-black uppercase tracking-widest text-muted-content mb-1">Requested
                                    By</label>
                            ...
                            <div class="pt-4 border-t border-border-color flex items-center justify-between">
                                <label
                                    class="text-[10px] font-black uppercase tracking-widest text-muted-content">Custom
                                    Series</label>
                                <span
                                    :class="series.custom ? 'bg-primary/10 text-button-content' : 'bg-info/10 text-info'"
                ...
                <div class="lg:col-span-2">
                    <div
                        class="bg-component-background rounded-3xl border border-border-color shadow-sm overflow-hidden">
                        <div
                            class="p-6 border-b border-border-color flex items-center justify-between bg-muted-background/50">
                            ...
                            <button @click="toggleAll"
                                class="text-[10px] font-black uppercase tracking-widest text-muted-content hover:text-primary transition-colors">
                            ...
                            <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-4">
                                <div v-for="clipper in series.clippers" :key="clipper.id"
                                    ...
                                    <div class="mt-2 text-center">
                                        <p class="text-[10px] font-black uppercase tracking-tighter text-muted-content">
                                            Slot #{{ clipper.series_number }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div
                            class="p-8 border-t border-border-color bg-muted-background/50 flex flex-col sm:flex-row gap-4">
                            <button @click="acceptFull"
                                class="flex-1 py-4 bg-primary text-button-content rounded-2xl font-black uppercase tracking-[0.2em] text-xs hover:bg-primary hover:text-button-content!  transition-all shadow-lg shadow-primary/20 active:scale-95 flex items-center justify-center gap-2">
                                <CheckCircle2 class="w-4 h-4" /> Accept All
                            </button>
                            <button @click="acceptPartial" :disabled="selectedClippers.length === 0"
        ...
        <DeclineModal :open="showDeclineModal" title="Decline Series Request"
        ...
        <ConfirmationModal :open="showAcceptFullModal" title="Accept All Clippers"
            description="Are you sure you want to accept this series and ALL its clippers?" confirmText="Accept All"
            @confirm="confirmAcceptFull" @cancel="showAcceptFullModal = false"
            @update:open="showAcceptFullModal = $event" :loading="form.processing" />

        <ConfirmationModal :open="showAcceptPartialModal" title="Accept Selected Clippers"
            :description="acceptPartialMessage" confirmText="Accept" @confirm="confirmAcceptPartial"
            @cancel="showAcceptPartialModal = false" @update:open="showAcceptPartialModal = $event"
            :loading="form.processing" />
    </AppLayout>
</template>
