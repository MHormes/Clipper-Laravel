    ...
    <AppLayout>
        <div class="max-w-7xl mx-auto p-6">
            <div class="mb-10">
                ...
                <p class="text-muted-foreground font-bold uppercase tracking-widest text-xs mt-1">Review additions to existing series</p>
            </div>

            <div v-if="Object.keys(groupedClippers).length === 0" class="bg-white dark:bg-[#161615] rounded-3xl p-20 border border-sidebar-border text-center shadow-sm">
                ...
                <p class="text-muted-foreground font-medium max-w-md mx-auto">All clipper requests have been processed. Great job!</p>
            </div>

            <div v-else class="space-y-12">
                <div v-for="(clippers, seriesId) in groupedClippers" :key="seriesId" class="space-y-6">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-xl overflow-hidden border border-sidebar-border shadow-sm">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] font-black uppercase tracking-widest text-muted-foreground mt-1">{{ clippers.length }} Pending Requests</p>
                        </div>
                    </div>

                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                        <div v-for="clipper in clippers" :key="clipper.id" 
                             class="bg-white dark:bg-[#161615] rounded-2xl border border-sidebar-border overflow-hidden group/item shadow-sm hover:border-orange-500/30 transition-all">
                            
                            <div class="aspect-[1/4] relative bg-gray-50 dark:bg-black overflow-hidden border-b border-sidebar-border">
                            ...
                            <div class="p-3">
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    <span class="text-[8px] font-bold text-muted-foreground">{{ formatDate(clipper.created_at) }}</span>
                                </div>
                                <p class="text-[10px] font-bold text-sidebar-foreground truncate uppercase tracking-tight">
