...
const mainNavItems = computed<NavItem[]>(() => {
    const items: NavItem[] = [
        ...
        },
        {
            ...
            icon: ListCheck,
            children: [
                { title: 'Your Clippers', href: '/clippers', icon: CheckCircle },
                { title: 'Your Series', href: '/my-series', icon: ListCheck },
    ...
    <Sidebar collapsible="icon" variant="inset">
        ...
        <div class="flex-1 overflow-y-auto py-2 space-y-2 transition-all duration-200"
            ...
            <div v-for="group in mainNavItems" :key="group.title" class="space-y-1">
                ...
                <div v-if="group.children && state !== 'collapsed'"
                    class="px-3 py-2 text-[10px] font-black uppercase tracking-widest opacity-70 text-muted-content">
                    {{ group.title }}
                </div>
                ...
                <div v-else class="space-y-1"
                    :class="state === 'collapsed' ? 'ml-0 border-l-0 pl-0' : 'ml-4 border-l-2 border-border-color pl-2 my-1'">
        ...
        <div class="px-2 pb-2" :class="state === 'collapsed' ? 'px-1' : 'px-2'">
            <InstallAppButton :full-width="state !== 'collapsed' || isMobile" />
        </div>
        ...
        <div class="p-2 border-t border-border-color">
            <DropdownMenu>
                <DropdownMenuTrigger as-child>
                    <button
                        class="flex w-full items-center gap-2 rounded-md p-2 text-left text-sm transition-colors hover:bg-muted-background focus:outline-none"
                ...
                <DropdownMenuContent class="w-56"
                    :side="isMobile ? 'bottom' : state === 'collapsed' ? 'right' : 'bottom'" align="end">
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem as-child>
                        <Link :href="logout()" method="post" as="button" @click="handleLogout"
